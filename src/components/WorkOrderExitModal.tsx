...
export function WorkOrderExitModal({
  isOpen,
  onClose,
  onSubmit,
  workOrder,
}: WorkOrderExitModalProps) {
  ...
  const handleSpecialtyChange = (
    specialty: keyof typeof formData.specialties
  ) => {
    setFormData({
      ...formData,
      specialties: {
  ...
  const handlePrecautionChange = (
    precaution: keyof typeof formData.precautions
  ) => {
    setFormData({
      ...formData,
      precautions: {
        ...
        >
          ...
          >
            <TabsList className="grid w-full grid-cols-4 gap-1 px-4 sm:px-6 pt-3 pb-2 h-auto p-0">
              ...
              <TabsTrigger value="tecnico" className="text-xs py-1">
                Técnico
              </TabsTrigger>
              <TabsTrigger value="materiales" className="text-xs py-1">
                Materiales
              </TabsTrigger>
            ...
            >
              ...
              <div className="bg-purple-50 p-2 rounded border border-purple-200 space-y-1">
                <h4 className="font-semibold text-xs p-2">
            ...
            >
              ...
              <div className="bg-orange-50 p-2 rounded border border-orange-200 space-y-1">
                ...
                <div className="grid grid-cols-2 gap-1">
                  <div>
                    <Label className="text-xs">Dotación de Personal</Label>
                    <Input
                      type="number"
                  ...
                  <div>
                    <Label className="text-xs">
                      Tiempo Total de la Operacion (h)
                    </Label>
                    <Input
            ...
            <TabsContent
              value="materiales"
              className="flex-1 overflow-hidden flex flex-col px-4 sm:px-6 py-3"
            >
                ...
                <table className="w-full text-xs border-collapse">
                  <thead>
                    <tr className="bg-gray-200">
                      ...
                      <th className="border p-1 text-center w-10">Cantidad</th>
                      <th className="border p-1 text-left">
                        Descripción/Codigo
                      </th>
                      <th className="border p-1 text-center w-6">Lubricado</th>
                      <th className="border p-1 text-center w-6">Calibrado</th>
                      <th className="border p-1 text-center w-6">Cambio</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="border p-0.5 text-center">
                          <Checkbox
                            ...
                            onCheckedChange={(checked: boolean) =>
                              handleMaterialChange(index, "lubricated", checked)
                            }
                        ...
                        <td className="border p-0.5 text-center">
                          <Checkbox
                            ...
                            onCheckedChange={(checked: boolean) =>
                              handleMaterialChange(index, "calibrated", checked)
                            }
                            className="w-3 h-3 mx-auto"
                          />
                        </td>
                        <td className="border p-0.5 text-center">
                          <Checkbox
                            ...
                            onCheckedChange={(checked: boolean) =>
                              handleMaterialChange(index, "replaced", checked)
                            }
                            className="w-3 h-3 mx-auto"
                          />
                        </td>
            ...
            >
              ...
              <div className="bg-yellow-100 border border-yellow-300 p-2 rounded">
                ...
                <p className="text-gray-700 text-xs">
                  Al realizar las tareas encomendadas en la presente orden de
                  trabajo deberán tener en cuanta las condiciones de seguridad
                  descritas a continuación que corresponda al caso.
                </p>
              </div>
              ...
              <div className="border border-gray-400 rounded p-2 bg-white">
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div className="space-y-1">
                    <h4 className="font-bold text-[11px] border-b">
                      PRECAUCIONES PRELIMINARES
                    </h4>

                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Despejar el área de
                      trabajo
                    </label>

                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Realizar una adecuada
                      señalización
                    </label>
                    ...
                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Utilizar protección
                      auditiva
                    </label>

                    <h4 className="font-bold text-[11px] border-b mt-2">
                      RIESGO MECANICO
                    </h4>

                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Utilizar caso de
                      seguridad
                    </label>

                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Utilizar guantes
                      protectores
                    </label>

                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Utilizar botas de
                      seguridad con punta de acero
                    </label>

                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Utilizar gafas de
                      seguridad
                    </label>

                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Utilizar ropa ajustada de
                      trabajo/ overol
                    </label>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Utilizar arnés de
                      seguridad si es en línea viva
                    </label>

                    <label className="flex items-center gap-1">
                      <Checkbox className="w-3 h-3" /> Utilizar ropa ignífuga
                    </label>
                  </div>
                  ...
                  {[
                    {
                      id: "technicianName",
                      label: "Técnico",
                      value: formData.signatures.technicianName,
                    },
                    {
                      id: "maintenanceChiefName",
                      label: "Jefe de Mantenimiento",
                      value: formData.signatures.maintenanceChiefName,
                    },
                    {
                      id: "plantChiefName",
                      label: "Jefe de Planta",
                      value: formData.signatures.plantChiefName,
                    },
                  ].map((sig) => (
                    <div key={sig.id}>
