...
export function WorkOrderClosingModal({
  ...
  const handleSecurityChange = (key: keyof typeof formData.securityRequirements) => {
    setFormData({
      ...formData,
      securityRequirements: {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[95vw] max-w-3xl max-h-[90vh] overflow-hidden flex flex-col p-0">
        <DialogHeader className="px-4 sm:px-6 py-3 border-b bg-blue-50">
          <DialogTitle className="text-base sm:text-lg font-bold">Cierre de OT: {workOrder?.id}</DialogTitle>
          <p className="text-xs text-gray-600 mt-1">FO – MTTO – 002 | Versión: 01 | Página: 1 de 1</p>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <Tabs defaultValue="general" className="flex-1 flex flex-col overflow-hidden">
            <TabsList className="grid w-full grid-cols-4 gap-1 px-4 sm:px-6 pt-3 pb-2 h-auto p-0">
              ...
              <TabsTrigger value="descripcion" className="text-xs py-1">Descripción</TabsTrigger>
              <TabsTrigger value="cierre" className="text-xs py-1">Cierre</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="general" className="flex-1 overflow-y-auto space-y-2 px-4 sm:px-6 py-3">
              <div className="grid grid-cols-3 gap-1">
                ...
                <div>
                  ...
                  <Input disabled value={new Date().toLocaleTimeString()} className="text-xs h-7 bg-gray-100" />
                </div>
              </div>
              ...
              <div className="bg-blue-50 p-2 rounded border border-blue-200 space-y-1">
                <h4 className="font-semibold text-xs">Identificación del Equipo</h4>
                <div className="grid grid-cols-2 gap-1">
                  <div>
              ...
              <div className="bg-red-50 p-2 rounded border border-red-200 space-y-1">
                ...
                <div className="grid grid-cols-2 gap-1">
                  {[
                    ...
                    <label key={prio.value} className="flex items-center gap-1 cursor-pointer text-xs">
                      <input
                        type="radio"
                        name="priority"
                        value={prio.value}
            ...
            <TabsContent value="seguridad" className="flex-1 overflow-y-auto space-y-2 px-4 sm:px-6 py-3">
              <div className="bg-orange-50 p-2 rounded border border-orange-200 space-y-1">
                <h4 className="font-semibold text-xs">Requisitos para la Ejecución - Seguridad</h4>
                <div className="space-y-1">
                  <div>
                    <p className="font-semibold text-xs mb-1">Requisito de Seguridad:</p>
                    <div className="grid grid-cols-2 gap-1">
                      {[
                        { key: 'loto', label: 'Aplicar Procedimiento LOTO (Bloqueo y Etiquetado)' },
                        ...
                        <label key={req.key} className="flex items-center gap-1 cursor-pointer text-xs">
                          <Checkbox
                            checked={formData.securityRequirements[req.key as keyof typeof formData.securityRequirements]}
                            onCheckedChange={() => handleSecurityChange(req.key as keyof typeof formData.securityRequirements)}
                            className="w-3 h-3"
                          />
                          {req.label}
                        </label>
                  ...
                  <div className="border-t pt-2">
                    <p className="font-semibold text-xs mb-1">Riesgos Identificados:</p>
                    <div className="grid grid-cols-2 gap-1">
                      {[
                        ...
                        <label key={risk.key} className="flex items-center gap-1 cursor-pointer text-xs">
                          <Checkbox
                            checked={formData.securityRequirements[risk.key as keyof typeof formData.securityRequirements]}
                            onCheckedChange={() => handleSecurityChange(risk.key as keyof typeof formData.securityRequirements)}
                            className="w-3 h-3"
                          />
                          {risk.label}
                        </label>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({ ...formData, otherRisks: e.target.value })}
            ...
            {/* TAB 3: DESCRIPCIÓN */}
            <TabsContent value="descripcion" className="flex-1 overflow-y-auto space-y-2 px-4 sm:px-6 py-3">
              <div>
                <Label className="text-xs">Descripción del Estado Antes de la OT</Label>
                <Textarea
              ...
              <div>
                <Label className="text-xs">Descripción de Efecto / Causa / No Prevista</Label>
                <Textarea
                  value={formData.effectCauseDescription}
                  onChange={(e) => setFormData({ ...formData, effectCauseDescription: e.target.value })}
              ...
              <div>
                <Label className="text-xs">Descripción del Trabajo Realizado *</Label>
                <Textarea
                  required
              ...
              <div>
                <Label className="text-xs">Hallazgos y Anomalías Encontradas</Label>
                <Textarea
              ...
              <div>
                <Label className="text-xs">Repuestos Utilizados (cantidad)</Label>
                <Textarea
              ...
              <div>
                <Label className="text-xs">Recomendaciones / Trabajos Pendientes</Label>
                <Textarea
            ...
            {/* TAB 4: CIERRE */}
            <TabsContent value="cierre" className="flex-1 overflow-y-auto space-y-2 px-4 sm:px-6 py-3">
                ...
                <div className="space-y-1">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({ ...formData, responsibleSignature: e.target.value })}
                      placeholder="Nombre"
                      className="text-xs h-7"
                    />
                  </div>
              ...
              <div className="bg-yellow-50 p-2 rounded border border-yellow-200 space-y-1">
                ...
                <div>
                  <Label className="text-xs">Trabajo Recibido y Validado Por *</Label>
                  <Input
                    required
                    value={formData.receivedValidatedBy}
                    onChange={(e) => setFormData({ ...formData, receivedValidatedBy: e.target.value })}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, validationSignature: e.target.value })}
                    placeholder="Nombre"
                    className="text-xs h-7"
                  />
                </div>
              ...
              <div className="bg-gray-50 p-2 rounded border space-y-1">
                <h4 className="font-semibold text-xs">Estado Final de la OT *</h4>
                <div className="flex gap-3 text-xs">
                  {[
                    { value: 'completado', label: 'Completado' },
                    { value: 'pendiente', label: 'Pendiente' },
                  ].map((status) => (
                    <label key={status.value} className="flex items-center gap-1 cursor-pointer">
                      <input
                        type="radio"
                        name="finalStatus"
                        value={status.value}
          ...
          <DialogFooter className="border-t px-4 sm:px-6 py-2 gap-2 bg-gray-50">
            ...
            <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-xs h-7">
              Cerrar OT
            </Button>
          </DialogFooter>
        </form>
